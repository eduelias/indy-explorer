<template>
    ...
    >
        ...
        <q-card-section class="glossy q-pa-none q-ma-none bg-grey-4 cursor-pointer" @click="openDialog(item)">
            <txn-metadata
        ...
        <q-card-section class="q-ma-none q-pa-sm bg-white">
            <q-chip size="sm" v-for="(service, index) in item.txn.data.data.services" :key="index">{{
                service
            }}</q-chip>
        </q-card-section>

        <q-expansion-item dense expand-icon-toggle class="text-grey-9 text-caption" expand-icon-class="text-grey-9">
            <template v-slot:header>
                <q-item-section avatar class="text-weight-bold">{{ item.txn.data.data.alias }}</q-item-section>
